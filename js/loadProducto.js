...
function loadNodeData(href){
  ...
  var jqxhr = $.get( hrefJson, function(data) {
    ...
    setTimeout(function(){
      ...
      //Cargar los datos
      setNodeData(jData, jData.nombre);
...
function setNodeData(data, title){
  ...
  $( infoProducto ).find( ".h1.producto b" ).text(data.nombre);
  $( infoProducto ).find( "img" ).attr("src", data.HOME + "basededatos/img/256_"+data.foto);
  $( infoProducto ).find( ".descripcion" ).text(data.descripcion);
  $( infoProducto ).find( ".tituloProduc" ).html(data.precio);
